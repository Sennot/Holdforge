...
bool HoldPopup::init(LevelEditorLayer* editor) {
    if (!Popup::init(440, 292, "GJ_square02.png")) return false;
    m_editor = editor;
    m_bgSprite->setColor({21, 29, 48});
    setTitle("HOLDFORGE", "bigFont.fnt", .65f, 21.f);
    m_title->setColor({106, 231, 221});
    label(m_mainLayer, "SILICATE  >  OPTIONS TRIGGERS", 220, 246, .29f, {148, 164, 194});
    auto card = NineSlice::create("GJ_square02.png"); card->setContentSize({404, 138});
    card->setColor({34, 45, 67}); card->setPosition({220, 165}); m_mainLayer->addChild(card);
    m_file = label(m_mainLayer, "Choose a .slc macro", 220, 215, .43f);
    m_stats = label(m_mainLayer, "Classic / dual / two-player", 220, 190, .31f, {148, 164, 194});
    label(m_mainLayer, "P1", 42, 158, .28f, {94, 230, 207}, 25);
    label(m_mainLayer, "P2", 42, 134, .28f, {175, 151, 255}, 25);
    m_timeline = CCDrawNode::create(); m_mainLayer->addChild(m_timeline);
    m_status = label(m_mainLayer, "1  Import    2  Analyze    3  Create", 220, 109, .29f);
    button(m_buttonMenu, this, menu_selector(HoldPopup::onImport), "Import .slc", 85, 72, 124, {49, 79, 103});
    button(m_buttonMenu, this, menu_selector(HoldPopup::onAnalyze), "Analyze", 220, 72, 124, {49, 79, 103});
    m_create = button(m_buttonMenu, this, menu_selector(HoldPopup::onCreate), "Create", 355, 72, 124, {29, 135, 123});
    ...
    button(m_buttonMenu, this, menu_selector(HoldPopup::onExport), "Export logs", 220, 31, 124, {40, 50, 74});
...
void HoldPopup::onCreate(CCObject*) {
    ...
    try {
        ...
        FLAlertLayer::create("HoldForge", fmt::format("<cg>{} Options Triggers created.</c>\nHold P1; in 2-player hold both inputs. Save after testing.{}", count, notes), "OK")->show();
    } catch (std::exception const& e) { failure(e); }
}
